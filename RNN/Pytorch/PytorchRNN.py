...
data = pd.concat([pd.read_csv(os.path.join('<Local>/SystematicStudy/TrainData', f)) for f in pa_files])
...
config = dict(    
    ...
    num_layers = 3,
    weight_decay = 1e-4,
    data_dir = '<Local>/SystematicStudy/TrainData'
    )
...
for run in range(10):
    ...
    # Building the model
    model = RNNModel(config['input_size'], config['hidden_size'], config['output_size'])
    ...
    # Training phase
    train_rmse, train_r2_score, test_rmse, test_r2_score = training_loop(train_loader, test_loader, device, config['epochs'], model, optimizer, criterion)
    ...
    print(f'Run {run + 1}: Train RMSE = {train_rmse:.4f}, Train R^2 = {train_r2_score:.4f}, Test RMSE = {test_rmse:.4f}, Test R^2 = {test_r2_score:.4f}, Time Elapsed = {elapsed_time:.2f} seconds')
...
# Save results to CSV
results_df.to_csv('<Local>/SystematicStudy/ExperimentResult/<file name>.csv', index=False)

# Print mean results
print('\nAggregated Results over 10 Runs:')
print(f'Mean Train RMSE = {mean_results["Train RMSE"]:.4f}, Mean Train R^2 = {mean_results["Train R2"]:.4f}, Mean Test RMSE = {mean_results["Test RMSE"]:.4f}, Mean Test R^2 = {mean_results["Test R2"]:.4f}, Mean Time Elapsed = {mean_results["Time Elapsed"]:.2f} seconds')
