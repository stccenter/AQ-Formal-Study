...
for(i in c(1:10)){
  ...
  time_start<-Sys.time()
  # DNN_model_pm <- neuralnet(epa_pm25 ~ ., data = train_x,
  #                                hidden = c(3,3,3),
  #                                linear.output = T,
  #                                learningrate = 0.001,
  #                                lifesign = 'full',stepmax =15000 )
  DNN_model_pm <- nnet(epa_pm25 ~ ., data = train_x,size=3,linout=TRUE)
  time_end<-Sys.time()
  time_tr_1<-time_end-time_start
  
  #save model
  write_rds(DNN_model_pm,str_c("/Users/ttrefoni/Documents/tt_pm25tuning/DNN_model_01_05_70_30_",i))
  
  #test data set
  time_start<-Sys.time()
  DNN_pred_pm <- predict(DNN_model_pm,test_x)
  time_end<-Sys.time()
  time_pred_1<-time_end-time_start
  
  DNN_metric_pm <- postResample(pred = DNN_pred_pm, obs = test_x$epa_pm25)
  
  DNN_mertric_df_pm <- data.frame(as.list(DNN_metric_pm)) 
  #bind to test results 
  test_results <- rbind(test_results,DNN_mertric_df_pm)
...
write_csv(test_sum,"/Users/ttrefoni/Documents/tt_pm25tuning/output/DNN_pm_results_01_05_70_30.csv")
# write to  file  ---------------------------------------------------------
file.create("/Users/ttrefoni/Documents/tt_pm25tuning/output/DNN_pm_results_01_05_70_30.txt")
file_con <- "/Users/ttrefoni/Documents/tt_pm25tuning/output/DNN_pm_results_01_05_70_30.txt"
